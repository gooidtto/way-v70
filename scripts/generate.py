...
inbounds=[
    inbound("node-01-xhttp",10086,"xhttp",path=XPATH),
    inbound("node-02-raw-reality",10087,"tcp","reality",RAW_SNI,RAW_TARGET,ids[0],"xtls-rprx-vision"),
    inbound("node-03-xhttp-reality",10088,"xhttp","reality",XHTTP_SNI,XHTTP_TARGET,ids[1],path=XPATH),
]
if CF_ENABLED:
    inbounds.append(inbound("node-04-cloudflare-ws",CF_PORT,"ws",path=CF["WS_PATH"]))
...
lines=[
    link(PUBLIC,443,{"encryption":"none","security":"tls","sni":PUBLIC,"fp":FP,"alpn":"h2,http/1.1","type":"xhttp","path":XPATH,"mode":"auto"},"Node 01 · Railway XHTTP TLS"),
    link(TCP_HOST,TCP_PORT,{"encryption":"none","flow":"xtls-rprx-vision","security":"reality","sni":RAW_SNI,"fp":FP,"pbk":PUB,"sid":ids[0],"type":"tcp"},"Node 02 · REALITY Vision · Railway TCP"),
    link(TCP_HOST,TCP_PORT,{"encryption":"none","security":"reality","sni":XHTTP_SNI,"fp":FP,"alpn":"h2","pbk":PUB,"sid":ids[1],"type":"xhttp","path":XPATH,"mode":"auto"},"Node 03 · XHTTP REALITY · Railway TCP"),
]
if CF_ENABLED:
    lines.append(link(CF["CLOUDFLARE_PUBLIC_HOSTNAME"],443,{"encryption":"none","security":"tls","sni":CF["CLOUDFLARE_PUBLIC_HOSTNAME"],"fp":FP,"alpn":"http/1.1","type":"ws","host":CF["CLOUDFLARE_PUBLIC_HOSTNAME"],"path":CF["WS_PATH"]},"Node 04 · Cloudflare WS TLS"))

count=len(lines)
routes={
    "node01":{"path":XPATH,"port":10086},
    "node02":{"sni":RAW_SNI,"port":10087,"short_id":ids[0]},
    "node03":{"sni":XHTTP_SNI,"port":10088,"short_id":ids[1]},
}
...
runtime={
    "schema":30,
    "build":"way-v70-standard-core",
    "architecture":"standard-three-node-core-plus-optional-node4",
    "nodes":{"count":count,"distribution":dist},
    "application_port":8080,
    "public_domain":PUBLIC,
    "tcp_proxy":{"domain":TCP_HOST,"port":TCP_PORT,"application_port":8080},
    "railway_networking":{"source":"current-deployment-environment","authoritative":True,"subscription_source":"current-runtime-values"},
    "cloudflare":{"enabled":CF_ENABLED,"public_hostname":CF["CLOUDFLARE_PUBLIC_HOSTNAME"] if CF_ENABLED else "","ws_port":CF_PORT if CF_ENABLED else None,"ws_path":CF["WS_PATH"] if CF_ENABLED else ""},
    "routes":routes,
}
...
print("RELEASE=way-v70-standard-core")
print("TOPOLOGY="+str(count))
print("NODE4_ENABLED="+str(CF_ENABLED).lower())
print("CLOUDFLARE="+('enabled' if CF_ENABLED else 'disabled'))
print("RAILWAY_CURRENT_PUBLIC="+PUBLIC)
print(f"RAILWAY_CURRENT_TCP={TCP_HOST}:{TCP_PORT}")
print("SUBSCRIPTION_SOURCE=current-runtime-values")
print("SUBSCRIPTION_COUNT="+str(count))
print("NODE_ORDER=01:RAILWAY_XHTTP,02:RAW_REALITY,03:XHTTP_REALITY"+(',04:CLOUDFLARE_WS' if CF_ENABLED else ''))
