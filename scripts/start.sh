...
[ -n "$PUBLIC" ] && [ -n "$TCP_HOST" ] && [ -n "$TCP_PORT" ] && [ -n "$APP_PORT" ] || { echo "FATAL: current Railway Public Networking/TCP Proxy values unavailable" >&2; exit 1; }
...
cf=bool(rt["cloudflare"]["enabled"]);expected=4 if cf else 3;n=int(rt["nodes"]["count"]);app=int(rt["application_port"])
...
if app!=int(rt["tcp_proxy"]["application_port"]): raise SystemExit("FATAL: Railway application port invariant failed")
...
print("TOPOLOGY_INVARIANT=OK");print("UUID_INVARIANT=OK");print("RAILWAY_NETWORKING_INVARIANT=OK");print("RAILWAY_APPLICATION_PORT="+str(app));print("SUBSCRIPTION_COUNT="+str(n));print("NODE_ORDER=01:RAILWAY_XHTTP,02:RAW_REALITY,03:XHTTP_REALITY"+(",04:CLOUDFLARE_WS" if cf else ""))
PY
