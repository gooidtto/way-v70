...
set -eu
umask 077
BUILD_ID="standard-core-dynamic-networking";SOURCE_BUILD="standard-core-dynamic-networking"
...
PUBLIC="${RAILWAY_PUBLIC_DOMAIN:-}";TCP_HOST="${RAILWAY_TCP_PROXY_DOMAIN:-}";TCP_PORT="${RAILWAY_TCP_PROXY_PORT:-}";APP_PORT="${RAILWAY_TCP_APPLICATION_PORT:-8080}"
[ -n "$PUBLIC" ]&&[ -n "$TCP_HOST" ]&&[ -n "$TCP_PORT" ]||{ echo "FATAL: current Railway Public Networking/TCP Proxy unavailable" >&2;exit 1; }
...
[ "$APP_PORT" = 8080 ]||{ echo "FATAL: TCP Proxy must target internal 8080, got $APP_PORT" >&2;exit 1; }
[ "$TCP_PORT" -ge 1 ]&&[ "$TCP_PORT" -le 65535 ]||exit 1
...
cfg=json.loads(Path(sys.argv[1]).read_text());sub=[x for x in Path(sys.argv[2]).read_text().splitlines() if x.strip()];u,rt,public,tcp_host,tcp_port=sys.argv[3],json.loads(Path(sys.argv[4]).read_text()),sys.argv[5],sys.argv[6],sys.argv[7];n=rt['nodes']['count'];cf=rt['cloudflare']['enabled']
expected=4 if cf else 3
if n!=expected or len(sub)!=expected:raise SystemExit(f'FATAL: topology/subscription mismatch runtime={n} subscription={len(sub)} expected={expected}')
...
for i,line in enumerate(sub,1):
 ...
 host,port=m.group(2),m.group(3);q=m.group(4)
 if i==1 and (host!=public or port!='443'):raise SystemExit(f'FATAL: node1 must use current Railway Public Domain: {host}:{port}')
 if i in (2,3) and (host!=tcp_host or port!=tcp_port):raise SystemExit(f'FATAL: node{i} must use current Railway TCP Proxy: {host}:{port}')
...
print('TOPOLOGY_INVARIANT=OK');print('UUID_INVARIANT=OK');print('RAILWAY_NETWORKING_INVARIANT=OK');print('SUBSCRIPTION_COUNT='+str(n));print('NODE_ORDER=01:RAILWAY_XHTTP,02:RAW_REALITY,03:XHTTP_REALITY'+(',04:CLOUDFLARE_WS' if cf else ''))
PY
xray run -test -config "$C";xray run -config "$C"&XP=$!;GP="";CFP="";trap 'kill "$XP" "$GP" "$CFP" 2>/dev/null||true;wait "$XP" 2>/dev/null||true;wait "$GP" 2>/dev/null||true;wait "$CFP" 2>/dev/null||true' INT TERM EXIT
...
python3 /opt/xray/scripts/gateway.py&GP=$!;waitp 8080 gateway
if [ "$CF" = 1 ];then CFPPORT=$(python3 - "$R" <<'PY'
import json,sys;print(json.load(open(sys.argv[1]))['cloudflare']['ws_port'])
PY
);secret "$D/cloudflare_tunnel_token.txt" "$CF_TOKEN";cloudflared --no-autoupdate tunnel --metrics 127.0.0.1:2000 run --token-file "$D/cloudflare_tunnel_token.txt">"$D/cloudflared.log" 2>&1&CFP=$!;sleep 1;kill -0 "$CFP" 2>/dev/null||{ tail -n 80 "$D/cloudflared.log" >&2;exit 1;};fi
...
PY
);echo "SOURCE_REPOSITORY=gooidtto/railway-v60-four-node";echo "SOURCE_BRANCH=main";echo "RELEASE=$BUILD_ID";echo "SOURCE_BUILD=$SOURCE_BUILD";echo "RAILWAY_CURRENT_PUBLIC=$PUBLIC";echo "RAILWAY_CURRENT_TCP=$TCP_HOST:$TCP_PORT";echo "TOPOLOGY=$N";echo "NODE4_ENABLED=$( [ "$CF" = 1 ]&&echo true||echo false )";echo "CLOUDFLARE=$( [ "$CF" = 1 ]&&echo enabled||echo disabled )";echo "SUBSCRIPTION_COUNT=$N";echo "TOPOLOGY_INVARIANT=OK";echo "NODE_ORDER=01:RAILWAY_XHTTP,02:RAW_REALITY,03:XHTTP_REALITY$( [ "$CF" = 1 ]&&echo ',04:CLOUDFLARE_WS'||true )"
while kill -0 "$XP" 2>/dev/null&&kill -0 "$GP" 2>/dev/null;do [ "$CF" = 0 ]||kill -0 "$CFP" 2>/dev/null||exit 1;sleep 5;done;exit 1