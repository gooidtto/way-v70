...
PY
); secret "$D/cloudflare_tunnel_token.txt" "$CF_TOKEN"; cloudflared --no-autoupdate tunnel --metrics 127.0.0.1:2000 run --token-file "$D/cloudflare_tunnel_token.txt" >"$D/cloudflared.log" 2>&1 & CFP=$!; sleep 1; kill -0 "$CFP" 2>/dev/null || { tail -n 80 "$D/cloudflared.log" >&2 || true; exit 1; }; waitp "$CFPPORT" cloudflare-origin; fi
