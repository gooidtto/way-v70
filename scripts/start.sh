...
if [ -s "$D/reality_private_key.txt" ] && [ -s "$D/reality_public_key.txt" ]; then
  PRIV=$(tr -d '[:space:]'<"$D/reality_private_key.txt"); PUB=$(tr -d '[:space:]'<"$D/reality_public_key.txt")
else
  OUT=$(xray x25519 2>&1); PRIV=$(printf '%s\n' "$OUT"|awk -F': ' '/^PrivateKey/{print $2;exit}'); PUB=$(printf '%s\n' "$OUT"|awk -F': ' '/^Password/{print $2;exit}'); [ -n "$PRIV" ] && [ -n "$PUB" ] || exit 1
  secret "$D/reality_private_key.txt" "$PRIV"; secret "$D/reality_public_key.txt" "$PUB"
fi
...
PY
  ...
  sleep 1; kill -0 "$CFP" 2>/dev/null || { tail -n 80 "$D/cloudflared.log" >&2 || true; exit 1; }
